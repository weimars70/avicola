...
export class GalponesService {
  ...
  async findOne(id: string): Promise<Galpon> {
  ...
  async update(id: string, updateGalponDto: UpdateGalponDto): Promise<Galpon> {
  ...
  async findAllIncludingInactive(): Promise<Galpon[]> {
    return await this.galponesRepository.find({
      order: { nombre: 'ASC' },
    });
  }
